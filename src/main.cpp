/*
	...
	You can't add objects from the ui, you must manually add them from the code

	Build:
		1- Install sbs in your system if not already downloaded (I'm sorry, I was young when I made this decision)
		2- type sbs all-run
	Controls:
		* Always:
			CTRL + ESC: Quit
		...
		* In menu:
			...
			T      : Adds/Removes the selected object from the trace list
			F      : Focus/Remove focus on the selected object
	UI:
		You can open/close the menu with ESC, every object in the universe is listed in the menu
		You can change the selected object by using keyboard arrows
		Pressing F will move the focus to the selected object
		if the selected object is already focused, then it will focus on nothing
		Pressing T will enable/disable tracing on the selected object

		If the focused object is traced, nothing will happen,
		because the relative movement between the focused object and camera is 0

		You can use the mouse to move around, and scroll to zoom in or out
*/
...
public:
	...
	std::string name;
public:
	physics_object(const std::string& pname, unit pmass, unit prad, olc::Pixel pcolor, const vec2& ipos, const vec2& ivel)
		: mass(pmass),
		  rad(prad),
		  pos(ipos),
		  color(pcolor),
...
public:
	physics_object* addto_universe(physics_object* obj, bool attract = true, bool isattracted = true)
	{
...
public:
	bool OnUserCreate() override
	{
		...
		physics_object* sun = addto_universe(new physics_object("Sun", SUN_MASS, 20.0, olc::YELLOW, vec2(0.0, 0.0), vec2(0, 0)), true, false);
		physics_object* earth = addto_universe(new physics_object("Earth", EARTH_MASS, 5.0, olc::CYAN, vec2(1000.0, 0.0), vec2(0.0, 0.0)), false, true);
		physics_object* mars = addto_universe(new physics_object("Mars", MARS_MASS, 1.2, olc::YELLOW, vec2(223.98 * 1000 / 147.53, 0.0), vec2(0.0, 0.0)), false, true);
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if(menu)
		{
			...
			{
				list<physics_object*>::iterator it = std::find(trackedobjs.begin(), trackedobjs.end(), universe[selected]);
				...
				else
				{
					...
					trackedobjs_pos.insert(std::pair<physics_object*, list<vec2>>(universe[selected], list<vec2>()));
				}
			}
		}
		if(!pause)
		...
		for(physics_object* tracked : trackedobjs)
		{
			if(tracked != nullptr)
			{
				...
				if(!pause)
				{
					physics_object fake("Fake", 0, 0, olc::WHITE, tracked->pos, vec2(0, 0));
					vec2 relpos;
					if(focus != nullptr)
						relpos = focus->relpos(fake);
					else
					{
						physics_object fake2("Fake", 0, 0, olc::WHITE, vec2(0, 0), vec2(0, 0));
				...
				for(const vec2& p : trackedpos)
				{
					Draw((p.x + offset.x) * zoom + ScreenWidth() / 2,
						-(p.y + offset.y) * zoom + ScreenHeight() / 2,
						tracked->color);
				}
			}
		}
		if(menu)
		{
			...
			for(physics_object* o : universe)
			{
				if(o->name.length() > largest_name && o->name.length() * 8 * TEXT_SCALE <= ScreenWidth() / 2)
					largest_name = o->name.length();
			}
			largest_name += 4; // Padding or " T F"
			FillRect(0, 0, largest_name * 8 * TEXT_SCALE, ScreenHeight(), olc::DARK_BLUE);
			FillRect(0, selected * 8 * TEXT_SCALE, largest_name * 8 * TEXT_SCALE, 8 * TEXT_SCALE, olc::BLUE);
			for(size_t i = 0; i < universe.size(); i++)
			{
				...
				if(focus == universe[i])
					name_stream << " F";
				DrawString(0, i * 8 * TEXT_SCALE, name_stream.str(), olc::WHITE, TEXT_SCALE);
			}
		}
